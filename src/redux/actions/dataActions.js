...
export const postFunkPost = (newFunkPost) => (dispatch) => {
	dispatch({ type: LOADING_UI });
	axios.post('/funkPost', newFunkPost)
		.then((res) => {
			dispatch({
				type: POST_FUNK_POST,
				payload: res.data
...
export const likeFunkPost = (funkPostId) => (dispatch) => {
	axios.get(`/funkPost/${funkPostId}/like`)
		.then((res) => {
			dispatch({
				type: LIKE_FUNK_POST,
				payload: res.data
...
export const unlikeFunkPost = (funkPostId) => (dispatch) => {
	axios.get(`/funkPost/${funkPostId}/unlike`)
		.then((res) => {
			dispatch({
...
export const deleteFunkPost = (funkPostId) => (dispatch) => {
	axios.delete(`/funkPost/${funkPostId}`)
		.then(() => {
			dispatch({
